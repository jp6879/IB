...
# ╔═╡ 1edf95dc-b74c-4d67-89ff-95ba7ef7853a
begin
	using DiffEqFlux, DifferentialEquations
	using Statistics, LinearAlgebra, Plots
	using Flux.Data: DataLoader, Flux
	using Optimization, OptimizationOptimJL
	using OptimizationFlux, Random, Plots
	using Flux: train!
	using Dates
end

# ╔═╡ fcc7c545-28d4-46c2-814a-0a0cdba1d402
#=╠═╡
md"file:///C:/Users/Propietario/Desktop/ib/5-Maestr%C3%ADa/Tesis/Julia/DiffEqFlux.jl/1-ANODE_Pluto.html"
  ╠═╡ =#

# ╔═╡ ce965ddd-531c-4ff9-94b6-da28bf05c77c
md"Intento de implenentar en Pluto el paper https://arxiv.org/pdf/1904.01681.pdf"

# ╔═╡ ee1a40aa-802b-40cf-8688-ca8c74ed537d
md"Las NODEs son una familia de redes neuronales que pueden interpretarse equivalentes contunuos de las ResNet. En las ResNet se mapean los datos de una entrada $x$ a una salida $y$ saltando entre capas de la red neuronal con una función $f$ que mantiene la dimensión. Luego se ajustaban los pesos de la red con una salida conocida $y_{true}$. Si suponemos una transformación de un estado oculto en una capa $t$ a una capa $t+1$ en las ResNet:"

# ╔═╡ 60635ff0-9930-45bd-9dc3-b78fdf999f0f
md"$h_{t+1} = h_t + f_t(h_t)$"

# ╔═╡ c3340e16-5466-445c-b4f0-eeb0f29cdfc3
md"Siendo $h_t \in \mathbb{R}^d$ el estado oculto en la capa $t$ y $f\::\mathbb{R}^{d} \to \mathbb{R}^{d}$ una función diferenciable que preserva la dimensión de $h_t$. Si se plantea esta diferencia como una discretizacion de la derivada podemos plantear"

# ╔═╡ 847b0fe4-daa3-4537-a7ea-9b9889d5891a
md"$\lim_{\Delta t \to 0} \frac{h_{t+\Delta t}-h_t}{\Delta t} = \frac{dh(t)}{dt} = f(h(t),t)$"

# ╔═╡ db73afe4-eb62-4aa4-b166-ea9f3b169959
md"Así el estado oculto puede ser parametrizado por una ecuación diferencial. Luego se pude mapear los puntos de datos $x$ en un conjunto de características $\phi(t)$ (supongo que se refiere a los parámetros de esa capa) resolviendo el problema"

# ╔═╡ 8802f845-893d-4544-85bb-117c699b5d02
md"$\frac{dh(t)}{dt} = f(h(t),t), \qquad h(0) = x$"

# ╔═╡ 1fccce3b-5c4f-4d12-8681-4ad06a55c218
md"hasta algún tiempo $T$. El estado oculto en el tiempo $T$, es decir $h(T)$, se corresponde con las características aprendidas por el modelo. Así las NODEs mapean una entrada $x$ a una salida $y$ resolviendo una ODE que parte de $x$. Luego se ajusta la dinámica del sistema que está dada por $f$ de manera que la ODE transforme $x$ en un $y$ cerca de $y_{true}$."

# ╔═╡ be2e26c7-2f64-46ac-a782-1ea1a6a95d2b
md"ODE FLOW: Se define el flujo del la ODE como el estado oculto en el tiempo t, es decir, $\phi_t(x) = h(t)$, cuando resolvemos la ODE en la condición $h(0) = x$. El flujo esencialente mide como los estados de las ODE dependen de la condición inicial $x$. Definimos las características de la ODE como $\phi(x) = \phi_T(x)$, es decir, el flujo en el tiempo final T al cual resolvemos la ODE."

# ╔═╡ dd4af829-ac5c-450a-9fe9-546bce6f22a7
md"NODE para regresiones y clasificación: Podemos utilizar ODEs para mapear datos de entrada $x \in \mathbb{R}^d$ a un conjunto de características o representaciones $\phi(x) \in \mathbb{R}^d$. Sin embargo, a menudo estamos interesados en aprender funciones de $\mathbb{R}^d$ a $\mathbb{R}$, por ejemplo, para regresión o clasificación. Para definir un modelo de $\mathbb{R}^d$ a $\mathbb{R}$, seguimos el ejemplo dado por Lin & Jegelka (2018) para las ResNets. Definimos el NODE $g : \mathbb{R}^d \rightarrow \mathbb{R}$ como $g(x) = L(\phi(x))$, donde $L : \mathbb{R}^d \rightarrow \mathbb{R}$ es una transformación lineal y $\phi : \mathbb{R}^d \rightarrow \mathbb{R}^d$ es la transformación de datos a características. Esta es una arquitectura de modelo simple: una capa ODE, seguida de una capa lineal."

# ╔═╡ c4204942-b02a-4f62-ab9b-a1f39f886dc1
md"En el trabajo se presentan funciones y flujos que las neural ODE no pueden representar. Por ejemplo para una dimensión arbitraria $d$ se puede encontar una clase de funciones definidas como $g\::\mathbb{R}^{d} \to \mathbb{R}^{d}$ tal que"

# ╔═╡ 4edce823-4e53-4648-81ea-4e4a1ba37fc1
md"$g(x) = \begin{cases} 
          -1 & 0 \leq ||x|| \leq r_1 \\
          1 & r_2 \leq ||x|| \leq r_3
       \end{cases}$"

# ╔═╡ c3dd16c3-7163-4191-af74-a330bcc19d89
md"Siendo $0<r_1<r_2<r_3$ no puede ser representada correctamente por una NODE."

# ╔═╡ 8ada996e-e501-43b3-b397-e7d9999d2711
md"Se propone una solución a este problema introduciendo las llmadas Argumented Neural ODEs. Estas proponen dimensiones extras al espacio en el cual aprende la red neuronal resolviendo la ODE de $\mathbb{R}^{d} \to \mathbb{R}^{d+p}$, permitiendo que el flujo de las ODE pase por dimensiones extra para evitar cruces en las trayectorias. De esta manera si $a(t) \in \mathbb{R}$ es un punto en la llamada parte argumentada del espacio, podemos formular el probelma equivalente"

# ╔═╡ 4f94f80f-1b4e-4d2d-9c97-0aeeceb15c2a
md"$\frac{d}{dt}
\begin{bmatrix}
h(t) \\
a(t)
...
# ╔═╡ f73090cd-1d70-4fc2-be70-7156ca95ea0e
md"Es decir a cada punto de los datos $x$ le agregamos ceros y resolvemos la ecuación diferencial en este espacio aumentado.
Vamos a implementar este ejemplo para una dimensión 2 así se puede graficar y ver los resultados obtenidos."

# ╔═╡ 2ba379a9-3fa6-4658-b040-f74acef2f5ab
begin
	# Random points in a circle of min_radius and max_radius of dimension 2
	function random_point_in_sphere(min_radius, max_radius)
	    θ = rand() * 2 * π
	    r = (max_radius - min_radius) .* (rand() .^ (1.0 / 2.0)) .+ min_radius
	    return [r * cos(θ), r * sin(θ)]
	end
end

# ╔═╡ a880fc8f-a03b-4fe8-b5a5-3a1e94604633
begin
	# Create the data of a concentric_sphere
	function concentric_sphere(inner_rad_range, out_rad_range,n_samples, batch_size)
	    data = []
	    labels = []
	    for _ in 1:n_samples
	        push!(data, reshape(random_point_in_sphere(out_rad_range...), :, 1))
	        push!(labels, ones(1, 1))
	    end
	    for _ in 1:n_samples
	        push!(data, reshape(random_point_in_sphere(inner_rad_range...), :, 1))
	        push!(labels, -ones(1, 1))
	    end
	    data = cat(data..., dims=2)
	    labels = cat(labels..., dims=2)
	    return DataLoader((data |> gpu, labels |> gpu); batchsize=batch_size, shuffle=true,
	    partial=false) # No se por que pero devolviendo los datos y los labels me tira error en la NeuralODE como si la dinámica fuese constante o algo así
	end
end

# ╔═╡ 06afeb22-db19-4d90-a3ff-0cbdc3477efc
begin
	diffeqarray_to_array(x) = reshape(gpu(x), size(x)[1:2])
	
	# Función para construir el modelo, haciendolo de otra forma sin estar en una funcion me tira error la NeuralODE
	function construct_model(out_dim, input_dim, hidden_dim, augment_dim)
	    ...
	    # En caso de no poner una dimensión argumentada no se usa y crea únicamente la neural ODE común
		if (augment_dim != 0)
	        node = AugmentedNDELayer(node, augment_dim)
	    end
	    return Chain((x, p=node.p) -> node(x, p), # No entiendo donde está la dinámica ¿Actualiza los parametros?
	                 Array,
...
# ╔═╡ fc899051-57c5-40b4-911c-85041e77dde6
begin
	# Generamos los datos para un circulo con radios $r_1 = 0,\:r_2 = 1,\: r_3 = 2,\:r_4 = 3$
	dataloader = concentric_sphere((0.0, 1.0), (2.0, 3.0), 2000 , 256)
	# Función loss o costo
	loss_node(x, y) = mean((model(x) .- y) .^ 2)
	
	# Creamos vectores para guardar los datos de loss y el tiempo en cada iteración
	lossNODE = []
	iter = 0
	times = []
	
	# Función de callback para guardar los valores y mostrar el progreso
	cb = function()
	    global iter
	    global lossNODE
	    iter += 1
	    actual_loss = loss_node(dataloader.data[1], dataloader.data[2])
	
	    # Guardamos el loss en cada iteración y la diferencia de tiempo
	    push!(times, now() - start)
	    push!(lossNODE, actual_loss)
	    
	    if iter % 10 == 0
	         println("Iteration $iter || Loss = $actual_loss")
	    end
	end
	# Entrenamos el modelo con una función callback para ver el progreso
	
	# Acá creamos el modelo de 3 capas 1 neurona de salida, 2 neuronas en la entrada y 128 neuronas en la capa oculta, en este caso no tenemos dimensión argumentada o sea es un modelo de NODE
	model, parameters = construct_model(1, 2, 128, 0)
	opt = ADAM(0.005)
	
	println("Training Neural ODE")
	# Hacemos el loop para entrenar la NODE, pasando los datos, la función loss, los parámetros y el modelo de la NODE, además de la función callback
	
	# Inicializamos el tiempo antes de empezar a entrenar
	start = now()
	push!(times, start - now())
	
	for _ in 1:20
		Flux.train!(loss_node, Flux.params(parameters, model), dataloader, opt, cb = cb)
	end
	time_in_s = [time.value * 1e-3 for time in times]
end

# ╔═╡ 377deba3-1e25-4b13-8ccc-f9e8a0836c42
md"Repitamos los mismos pasos para un modelo con una dimensión argumentada distinta de 0"

# ╔═╡ f7ee11ef-55c9-48f5-a25c-0eec58ed8b14
begin
	# Creamos vectores para guardar los datos de loss en cada iteración
	iter2 = 0
	lossANODE = []
	# Función loss o costo
	loss_anode(x, y) = mean((model2(x) .- y) .^ 2)
	# Función de callback para guardar los valores y mostrar el progreso de la ANODE
	cb2 = function()
		global iter2
		global lossANODE
		actual_loss = loss_anode(dataloader.data[1], dataloader.data[2])
		iter2 += 1
		
		push!(lossANODE, actual_loss)
		# No guardo el tiempo para poner los gráficos en el mismo eje asumiendo que deben ser similare
		if iter2 % 10 == 0
			println("Iteration $iter2 || Loss = $actual_loss")
		end
	end
	
	# Creamos el modelo con las mismas carácteristicas que el anterior pero con una dimesión argumentada 64 (parecen demasiadas dimensiones extra pero funciona bien)
	model2, parameters2 = construct_model(1, 2, 128, 64)
	opt2 = ADAM(0.005)

	
	println("Training Argumented Neural ODE")
	for _ in 1:20
		Flux.train!(loss_anode, Flux.params(parameters2, model2), dataloader, opt, cb = cb2)
	end
end

# ╔═╡ 353f0c52-8380-4f75-bdef-d3eafb5cc46d
begin 
	# Función que genera datos aleatorios en una circunferencia de radio 3 y devuelve un mapa con los valores que predice el modelo que le pasamos
	function plot_contour(model, npoints = 300)
	    grid_points = zeros(2, npoints ^ 2)
	    idx = 1
	    x = range(-3.0, 3.0, length = npoints)
	    y = range(-3.0, 3.0, length = npoints)
	    for x1 in x, x2 in y
...
# ╔═╡ 14ac2819-7d24-47c7-897f-d990545803d4
# Grafico de lo que predice la NODE
plt_node = plot_contour(model)

# ╔═╡ 948d0328-0e0c-4579-b9d3-fd26e3af3ccf
# Grafico de lo que predice la ANODE
plt_anode = plot_contour(model2)

# ╔═╡ e4bee82e-0a0e-482e-84f5-3446faf43058
begin
	# Grafico del loss o costo en función del tiempo
	plot(time_in_s[2:end], lossNODE, label = "NODE", xlabel = "Time (s)", ylabel = "Loss", title = "Loss vs Time", legend = :topleft)
	plot!(time_in_s[2:end],lossANODE, label = "ANODE")
end

# ╔═╡ c4d2906b-3084-41de-ab5f-0c9e7cfeb9b7
md"Se ve como la función loss se minimiza mucho mas rápido para una ANODE que para una NODEm además la ANODE representa correctamente para un conjunto de datos aleatorios el comportamiento de la funcion $g$ para las mismas epocas de entrenamiento que en este caso fueron únicamente 20. Si se sigue entrenando la ANODE esta logra una mejor representación de la función $g$"
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "c0e5d98a97aa87538e735aea738451c470f6b818"
...
git-tree-sha1 = "bc7aae67da9e60ca177da428b2f84909be6b7bd3"
...
[[deps.LuxCore]]
deps = ["DocStringExtensions", "Functors", "Random", "Setfield"]
git-tree-sha1 = "f2dafe0ddcecf06247b40dbf336acd14e0adce6d"
...
git-tree-sha1 = "9be77b6e50986b0652170ae7f656ae2f037a8179"
...
git-tree-sha1 = "b7187c3cd64dea835fe20b612f8a3e1ca02fb3c1"
...
# ╔═╡ Cell order:
# ╠═99a93a1d-94ea-4d8f-98c0-fd696a545db8
# ╠═fcc7c545-28d4-46c2-814a-0a0cdba1d402
# ╟─ce965ddd-531c-4ff9-94b6-da28bf05c77c
# ╟─ee1a40aa-802b-40cf-8688-ca8c74ed537d
# ╟─60635ff0-9930-45bd-9dc3-b78fdf999f0f
